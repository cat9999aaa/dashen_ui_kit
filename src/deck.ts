...
document.querySelectorAll<HTMLElement>("[data-deck-next]").forEach((button) => {
  button.addEventListener("click", () => setSlide(activeIndex + 1));
});

document.querySelectorAll<HTMLElement>("[data-deck-prev]").forEach((button) => {
  button.addEventListener("click", () => setSlide(activeIndex - 1));
});
